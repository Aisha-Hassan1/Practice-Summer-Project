...
document.forms[0].onsubmit = function(e){
        ...
        for(let j=0;j<counterStores;j++)
        {
            
            let myRequest = new XMLHttpRequest();
                myRequest.open("GET", "Info_of_Stores.json");
                myRequest.send();
                ...
                myRequest.onreadystatechange = function () 
                {
                    ...
                    let jsData = JSON.parse(this.responseText);
                        ...
                        if(jsData[j].icy_orders!=null)
                        {
                            let total_orders= +(jsData[j].total_orders);
                            let icy_orders=+(jsData[j].icy_orders);
                            let not_icy_orders= total_orders - icy_orders;
                            stores[j]=
                            { 
                                Name:`Store ${jsData[j].id}`,
                                "total":total_orders,
                                "icy":icy_orders,
                                "notIcy":not_icy_orders,
                            };
                            sessionStorage.setItem('Stores', JSON.stringify(stores));
                        }
                        else{
                            let total_orders= +(jsData[j].total_orders);
                            let not_icy_orders= total_orders ;
                            stores[j]=
                            { 
                                Name:`Store ${jsData[j].id}`,
                                "total":total_orders,
